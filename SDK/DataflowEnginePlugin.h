
/********************************************************
*                                                       *
*   Package generated using UEDumper by Spuckwaffel.    *
*                                                       *
********************************************************/

/// Package DataflowEnginePlugin.

/// Class /Script/DataflowEnginePlugin.DataflowActor
/// Size: 0x0008 (0x000290 - 0x000298)
class ADataflowActor : public AActor : AActor
{ 
public:
	UDataflowComponent*                                DataflowComponent;                                          // 0x0290   (0x0008)  
};

/// Class /Script/DataflowEnginePlugin.DataflowComponent
/// Size: 0x0140 (0x000570 - 0x0006B0)
class UDataflowComponent : public UPrimitiveComponent : UPrimitiveComponent
{ 
public:
	unsigned char                                      UnknownData00_1[0x140];                                     // 0x0570   (0x0140)  MISSED
};

